...
void UpdateHandSkeletonBones(bool isLeftHand)
{
    ...
    foreach (HumanBodyBones boneKey in handBoneIdMap.Keys)
    {
        ...
        if ((int)ovrBoneId < skeleton.Bones.Count)
        {
            ...
            Transform targetBone = _Skeleton[boneKey];
            
            // ワールド回転をコピーしてからローカルに変換
            Quaternion worldRotation = skeletonBone.rotation;
            
            if (targetBone.parent != null)
            {
                targetBone.localRotation = Quaternion.Inverse(targetBone.parent.rotation) * worldRotation;
            }
            else
            {
                targetBone.localRotation = worldRotation;
            }
        }
    }
}
